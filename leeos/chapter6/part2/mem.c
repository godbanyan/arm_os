...
int kmem_cache_line_object(void *head, unsigned int size, int order)
{
	...
	*pl = NULL;			// head of last block point to NULL
...
#define KMALLOC_MINIMAL_SIZE				(1 << KMALLOC_BIAS_SHIFT)
#define KMALLOC_CACHE_SIZE					(KMALLOC_MAX_SIZE / KMALLOC_MINIMAL_SIZE)
...
int kmalloc_init(void)
{
	...
	for(i = 0; i < KMALLOC_CACHE_SIZE; i++) {
		if(kmem_cache_create(&kmalloc_cache[i], (i + 1) * KMALLOC_MINIMAL_SIZE, 0) == NULL)
			return -1;
	}
